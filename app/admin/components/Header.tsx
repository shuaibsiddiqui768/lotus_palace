...
export function Header() {
    ...
    try {
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        if (initializedRef.current) {
          const hasNew = unreadIds.some((id) => !previousIdsRef.current.includes(id));
          if (hasNew) setHasNewNotification(true);
          else if (unreadIds.length === 0) setHasNewNotification(false);
        } else {
        ...
        if (unreadIds.length === 0) setHasNewNotification(false);

        setNotifications(unread);
        broadcastUnreadCount(unread.length);
      }
    } catch (error) {
      if (isMountedRef.current) console.error('Error loading notifications', error);
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) dismissedNotificationsRef.current = new Set(parsed);
    } catch {
      dismissedNotificationsRef.current = new Set();
      persistDismissedNotifications();
    }
  ...
  const handleMarkAllAsRead = useCallback(() => {
    ...
    notifications.forEach((n) => dismissedNotificationsRef.current.add(n.id));
    setNotifications([]);
  ...
  const handleDismissNotification = useCallback(
    (id: string) => {
      ...
      setNotifications((previous) => {
        ...
        previousIdsRef.current = previousIdsRef.current.filter((prevId) => prevId !== id);
        if (next.length === 0) setHasNewNotification(false);
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const handleOrdersOpened = () => handleMarkAllAsRead();
  ...
  return (
    <header className="border-b border-emerald-100 bg-gradient-to-r from-emerald-50 via-white to-emerald-50 sticky top-0 z-10">
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
        ...
        <div className="flex items-center gap-2 md:gap-4 ml-10 md:ml-0"></div>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          >
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className={`relative h-8 w-8 ${hasNewNotification ? 'text-emerald-700' : ''}`}
              >
                <Bell size={18} />
            ...
            <DropdownMenuContent align="end" className="w-80">
              ...
              <div className="max-h-72 overflow-y-auto">
                {notifications.length === 0 ? (
                  <div className="px-3 py-6 text-sm text-emerald-700">No new orders</div>
                ) : (
          ...
          {/* USER ICON */}
          <Button variant="ghost" size="icon" className="h-8 w-8 text-emerald-700 hover:bg-emerald-50">
            <User size={18} />
          </Button>
