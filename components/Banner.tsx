...
const Banner = ({ onOrderClick, onViewMenuClick }: BannerProps) => {
  ...
  return (
    <section className="relative min-h-[60vh] sm:min-h-screen flex items-center justify-start overflow-hidden w-full shadow-lg">
      {/* Background Slides - pointer-events-none so arrows work */}
      ...
      <div className="relative w-full px-6 sm:px-12 md:px-16 lg:px-24 z-10">
        <div className="max-w-2xl space-y-6 text-left">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 pt-2">
            <Button
              size="lg"
              className="gap-2 shadow-xl bg-gradient-to-r from-emerald-600 to-lime-600 hover:from-emerald-700 hover:to-lime-700 
              text-white font-semibold px-6 sm:px-8 py-5 sm:py-6 text-base sm:text-lg transition-all duration-300 hover:scale-105 border border-white/20"
            ...
            <Link href="/menu">
              <Button
                size="lg"
                variant="outline"
                className="bg-white/15 backdrop-blur-md border-2 border-white/30 text-white hover:bg-white/25 
                hover:border-white/50 font-semibold px-6 sm:px-8 py-5 sm:py-6 text-base sm:text-lg transition-all duration-300 hover:scale-105 shadow-lg"
              >
                View Menu
              </Button>
            </Link>
      ...
      {/* Slide indicators */}
      <div className="absolute bottom-6 sm:bottom-8 left-1/2 -translate-x-1/2 z-30 flex gap-2 sm:gap-3 pointer-events-auto">
        {slides.map((_, index) => (
          <button
            key={index}
            type="button"
